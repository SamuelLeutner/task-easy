      ...
      <div class="account-setup">
        <div class="form">
          <DynamicButtonComponent
            :title="'Selecione o que deseja gerenciar primeiro'"
            :subTitle="'Você poderá adicionar outras opções posteriormente'"
            :buttons="buttonsManagement"
            @updateSelected="selectedManagement"
            name="team"
          />
        </div>
...
export default defineComponent({
  ...
  computed: {
    ...mapGetters("user_type", ["getManagementArea"]),
  },
  methods: {
    selectedManagement(target) {
      this.buttonActive = true;
      this.$store.dispatch("user_type/saveManagementArea", target);
    },
  ...
  setup() {
    const buttonsManagement = [
      { text: "TI", nameInput: "options", label: "1" },
      { text: "Marketing", nameInput: "options", label: "2" },
      { text: "Jurídico", nameInput: "options", label: "3" },
      { text: "Educação", nameInput: "options", label: "4" },
      { text: "RH e recrutamento", nameInput: "options", label: "5" },
      { text: "Vendas e CRM", nameInput: "options", label: "6" },
      { text: "Design e criação", nameInput: "options", label: "7" },
      { text: "Finanças", nameInput: "options", label: "8" },
    ];
    return {
      FormTypes,
      buttonsManagement,
